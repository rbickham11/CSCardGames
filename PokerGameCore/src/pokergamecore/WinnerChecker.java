...
public class WinnerChecker 
{
            ...
            switch(rank)
            {
                ...
                case 8: //Straight Flush
                    for(j = sevenCardHand.size() - 1; j >= 4; j--)
                    {
                        if(getSuit(sevenCardHand.get(j)) == getSuit(sevenCardHand.get(j - 4)))
                        {
                            if(getCardValue(sevenCardHand.get(j)) == getCardValue(sevenCardHand.get(j - 4)) + 4)
                            {
                                for(k = j; k > j - 5; k--)
                                {
                                    fiveCardHands.add(getCardValue(sevenCardHand.get(k)));
    ...
    public boolean isStraight(List<Integer> hand)
    {
        ...
        if(distinctHand.get(distinctHand.size() - 1) == 12 && distinctHand.get(0) == 0 && distinctHand.get(1) == 1 && distinctHand.get(2) == 2 && distinctHand.get(3) == 3) //Checking for A-2-3-4-5 straight
        {
            return true;
        }
